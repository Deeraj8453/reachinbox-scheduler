...
import { Search, Plus, MoreVertical, Calendar, Send } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { data: scheduledData, isLoading: isLoadingScheduled, refetch: refetchScheduled } = useQuery<PaginatedEmails>({
    ...
    refetchInterval: () => {
      ...
      return activeTab === 'scheduled' ? 5000 : false;
    }
  });
  ...
  const { data: sentData, isLoading: isLoadingSent } = useQuery<PaginatedEmails>({
    ...
    refetchInterval: () => {
      if (document.visibilityState !== 'visible') return false;
      return activeTab === 'sent' ? 5000 : false;
    }
  });
                ...
                <AnimatePresence mode="wait">
                  {activeTab === 'scheduled' && (
                    isLoadingScheduled ? (
                      ...
                      scheduledData?.emails.map((job: Email) => (
                        <tr key={job.id} onClick={() => setSelectedJob(job)} className="border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer group">
                          <td className="py-4 px-6">
                            <span className="text-sm font-medium text-white">{getLocalPart(job.recipient)}</span>
                          </td>
                          <td className="py-4 px-6">
